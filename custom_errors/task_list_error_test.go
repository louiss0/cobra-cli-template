...
var _ = Describe("task list errors", func() {
	It("formats not signed in errors with sign-in guidance", func() {
		...
		assert.Contains(err.Error(), "task-list error: sign-in required")
		assert.Contains(err.Error(), "task-list auth signin <username>")
	})
	...
	It("formats unknown user errors with register guidance", func() {
		...
		assert.Contains(err.Error(), "task-list error: user not found: alice")
	...
	It("formats task not found errors with list guidance", func() {
		...
		assert.Contains(err.Error(), "task-list error: task not found: task-404")
		assert.Contains(err.Error(), "task-list list")
	})
	...
	It("wraps args validation errors using the task list formatter", func() {
		...
		rootCommand := &cobra.Command{Use: "task-list"}
		...
		assert.Error(err)
		assert.Contains(err.Error(), "task-list error: accepts 1 arg(s), received 0")
