...
When building you should use the `go build --ldflags "-X github.com/louiss0/g-tools/mode.buildMode=production" -o taskman.exe` command.

Prefer building with that command instead of running tests for routine
verification unless the user explicitly asks for tests.
